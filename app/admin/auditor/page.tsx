  ...
  return (
    ...
    >
      <div style={{ maxWidth: 1100, margin: "0 auto" }}>
        ...
        <div style={{ marginBottom: 16, padding: 12, borderRadius: 12, backgroundColor: "#ffffff", border: "1px solid #e5e7eb" }}>
          <label style={{ fontSize: 13, fontWeight: 500 }}>稽核人名稱</label>
          <input value={auditorName} onChange={(e) => setAuditorName(e.target.value)} style={inputStyle} />
        ...
        <div style={{ borderRadius: 16, backgroundColor: "#ffffff", border: "1px solid #e5e7eb", overflow: "hidden" }}>
          <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 13 }}>
            ...
            <tbody>
                ...
                <tr key={b.id} style={{ borderBottom: "1px solid #e5e7eb" }}>
                  ...
                  <td style={tdStyle}>
                    <div style={{ display: "flex", gap: 4, flexWrap: "wrap" }}>
                      <button disabled={updatingId === b.id} onClick={() => updateStatus(b.id, "approved")} style={{ ...actionBtnStyle, backgroundColor: "#16a34a", color: "#fff" }}>
                        通過
                      </button>
                      <button disabled={updatingId === b.id} onClick={() => updateStatus(b.id, "rejected")} style={{ ...actionBtnStyle, backgroundColor: "#dc2626", color: "#fff" }}>
                        退回
                      </button>
                      <button disabled={updatingId === b.id} onClick={() => updateStatus(b.id, "pending")} style={{ ...actionBtnStyle, backgroundColor: "#e5e7eb", color: "#374151" }}>
                        待審
                      </button>
                    </div>
                  </td>
              ...
              {batches.length === 0 && (
                <tr>
                  <td colSpan={9} style={{ padding: 10, textAlign: "center", color: "#6b7280" }}>
                    目前尚無任何批次資料。
                  </td>
                </tr>
              )}
        ...
        {message && (
          <p style={{ marginTop: 10, fontSize: 12, color: message.startsWith("更新失敗") ? "#dc2626" : "#16a34a", whiteSpace: "pre-wrap", fontWeight: 700 }}>
            {message}
          </p>
        )}
      </div>
