// app/api/processor/sign/route.ts
...
export async function POST(req: Request) {
  try {
    ...
    const signerName = String(body?.signerName || body?.by || "Processor").trim();
    const note = String(body?.note || "processor signed").trim();
    ...
    const priv =
      readPemFromEnv("PROCESSOR_PRIVATE_KEY_PEM") ||
      readPemFromEnv("PRIVATE_KEY_PEM"); // fallback（demo）

    if (!priv) {
      return NextResponse.json(
        { ok: false, error: "PRIVATE_KEY_MISSING", hint: "Set PROCESSOR_PRIVATE_KEY_PEM" },
        { status: 500 }
      );
    }
    ...
    const kid = String(process.env.PROCESSOR_KID || "processor-key-1");
    const did = String(process.env.PROCESSOR_DID || "did:web:processor.local");
    ...
    const nextSignatures = signatures.filter((s) => String(s?.role || "") !== "processor");
    nextSignatures.push(sigObj);

    const events: any[] = Array.isArray(v?.events) ? [...v.events] : [];
    events.push(buildSignEvent({ role: "processor", by: signerName, note, tsIso: nowIso, kid }));
    ...
    return NextResponse.json({
      ...
      signer: { role: "processor", signerName, did, kid, signedAt: nowIso },
    });
  } catch (err: any) {
    return NextResponse.json(
      { ok: false, error: "PROCESSOR_SIGN_FAILED", message: String(err?.message || err) },
      { status: 500 }
    );
  }
}
...
/**
 * GET /api/processor/sign?batchId=...
 */
export async function GET(req: Request) {
  const url = new URL(req.url);
  const batchId = url.searchParams.get("batchId") || "";
  const signerName = url.searchParams.get("signerName") || "Processor";
  const note = url.searchParams.get("note") || "processor signed";
